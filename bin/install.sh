#!/usr/bin/env bash
REPO_NAME="terraform-provider-zipper"
NAME="terraform-provider-zipper"
OS=""
OWNER="ArthurHlt"
...
provider_path="${HOME}/.terraform.d/providers/terraform-provider-zipper"
grep -Fxq "providers {" ~/.terraformrc &> /dev/null
if [[ $? != 0 ]]; then
    cat <<EOF >> ~/.terraformrc
providers {
    zipper = "$provider_path"
}
EOF
else
